    ...
    <div className="flex min-h-screen flex-col bg-background">
      <header className="border-b">
        <div className="mx-auto flex w-full max-w-4xl items-center justify-between gap-4 px-6 py-5">
          <Link
            href={`/faculty/${summary.facultySlug}`}
            className="inline-flex items-center gap-2 text-sm text-muted-foreground transition-colors hover:text-foreground"
          >
            <ArrowLeft className="size-4" />
            Back to {summary.facultyName}
          </Link>
      ...
      <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col px-6 py-10 md:py-14">
        ...
        <section className="mb-10 space-y-6 rounded-2xl border bg-card p-6 md:p-8">
          <div className="space-y-2 text-center sm:text-left">
            <div className="mx-auto flex size-12 items-center justify-center rounded-full border sm:mx-0">
        ...
        {!sheet ? (
          <section className="mx-auto w-full max-w-lg rounded-2xl border bg-card p-6 md:p-8">
            <div className="mb-6 space-y-2">
              <div className="flex items-center gap-2 text-muted-foreground">
                <KeyRound className="size-4" />
                <p className="text-sm">Answer sheet</p>
              </div>
              <h2 className="text-xl font-semibold tracking-tight">
                Enter your access code
              </h2>
              <p className="text-sm leading-6 text-muted-foreground">
                Use the same one-time code you used to take this quiz to view
...
function AnswerSheetView({ sections }: { sections: AnswerSheetSection[] }) {
  return (
    <section className="space-y-10">
      <div className="space-y-2">
        <h2 className="text-xl font-semibold tracking-tight">Answer sheet</h2>
        <p className="text-sm text-muted-foreground">
          Green means you picked the right option. Red means your choice was
          wrong — the correct answer is outlined.
        </p>
        <div className="flex flex-wrap gap-3 pt-1 text-xs text-muted-foreground">
          <Legend swatch="bg-emerald-500/15 border-emerald-600/40" label="Your correct pick" />
          <Legend swatch="bg-red-500/10 border-red-600/40" label="Your wrong pick" />
          <Legend swatch="border-emerald-600 border-2 bg-background" label="Correct answer" />
        </div>
      </div>
...
function QuestionReview({ question }: { question: AnswerSheetQuestion }) {
  return (
    <article className="rounded-2xl border bg-card p-5 md:p-6">
      ...
      <h4 className="mt-2 text-base font-semibold leading-7 md:text-lg">
        {question.prompt}
      </h4>
...
function OptionRow({
  ...
  return (
    <div
      className={cn(
        "flex w-full items-start gap-3 rounded-xl border px-4 py-3.5 text-left",
        selectedCorrect &&
        ...
        correctUnselected &&
          "border-2 border-emerald-600 bg-emerald-500/5",
        !option.isSelected &&
          !option.isCorrect &&
