  ...
  try {
    ...
    if (updated.length === 0) {
      return actionFailure(
        "Could not mark this code as issued. Please try again.",
      );
    }
    ...
    console.error("markAccessCodeIssued failed:", error);
...
export async function releaseAccessCode(
  ...
  try {
    ...
    return actionSuccess(
      undefined,
      `Released ${existing.code} back to available.`,
    );
  } catch (error) {
    console.error("releaseAccessCode failed:", error);
    return actionFailure("Could not release this code. Please try again.");
  }
}
